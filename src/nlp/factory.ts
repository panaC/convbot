...
type Tgraph<Tname extends string, Tcontext extends string, Tconv> = {
  [n in Tname]?: Inlp<Tname, Tcontext, Tconv>
};

export const nlpFactory = async <
  Tname extends string,
  Tcontext extends string,
  Tconv,
  Tdata extends InlpData<Tname, Tcontext, Tconv>
>(
  graph: Tgraph<Tname, Tcontext, Tconv>
) => {
  const classifier = new NluManager({
    languages: Object.entries<Inlp<Tname, Tcontext, Tconv>>(graph as {
      [s: string]: Inlp<Tname, Tcontext, Tconv>;
    }).reduce(
      (p, c) => [
        ...new Set([
          ...p,
  ...
  Object.entries<Inlp<Tname, Tcontext, Tconv>>(graph as {
    [s: string]: Inlp<Tname, Tcontext, Tconv>;
  }).map(o => {
